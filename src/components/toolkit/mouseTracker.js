...
import { imgCanvas as canvas, imgCtx as ctx } from '/src/components/canvas/canvasExport.js'
import { annotatorEl, coordinateIndex } from '/src/components/modules/elements.js'
...
export default function chaseWholeCanvasCoordinate(img) {
  ...
  function onMouseMove(e) {
    ...
    const fixedSecondDecimalPoint = (computedValue) => Number(computedValue.toFixed(2))

    CoordStore.canvas.guideLine.x = x
    CoordStore.canvas.guideLine.y = y
    CoordStore.img.guideLine.x = fixedSecondDecimalPoint(CoordStore.canvas.guideLine.x - CoordStore.img.translation.x)
    CoordStore.img.guideLine.y = fixedSecondDecimalPoint(CoordStore.canvas.guideLine.y - CoordStore.img.translation.y)
    ...
    CoordStore.img.scale.x = fixedSecondDecimalPoint((canvas.width * scale) / imgWidth)
    CoordStore.img.scale.y = fixedSecondDecimalPoint((canvas.height * scale) / imgHeight)
    CoordStore.img.scaledSize.width = fixedSecondDecimalPoint(CoordStore.img.size.width * scale)
    CoordStore.img.scaledSize.height = fixedSecondDecimalPoint(CoordStore.img.size.height * scale)
